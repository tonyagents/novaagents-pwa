# TripPilot — autonomous travel-booking agent (Paybox demo)
...
```
 ...
 Node server ── Claude Agent SDK
                    ...
                    └── paybox  (remote MCP @ api.paybox.sh/mcp, OAuth bearer)
```

The **travel** inventory is curated/static so the demo is reliable live and on video. The **real** artifacts are the Paybox card + passkey approval.
...
`paybox-auth.mjs` registers a client, runs the OAuth flow, and saves an access + refresh token to `.paybox-token.json` (gitignored). You only do this once; the token auto-refreshes.
...
`start.sh` loads the Paybox token, prints an **app access token**, and starts the server on `:8790`. Open **http://localhost:8790**, paste the token once. (If `cloudflared` is installed, it also opens a phone tunnel.)
...
3. Click **🔐 Approve with passkey** → Paybox approval page → biometric. The card flips to **Approved ✓** and a **Booked** confirmation shows the masked one-time card.
...
## How the pieces map to Paybox tools
...
| File | What |
|---|---|
| `server.mjs` | Express + Agent SDK; `travel` + `paybox` MCP servers; SSE forwards tool name/input/output |
| `travel-inventory.mjs` | Curated flights + hotels (merchant, url, price) |
| `paybox-auth.mjs` | One-time Paybox OAuth 2.1 + PKCE login; `--print` / `--refresh` helpers |
| `public/` | Web app: chat + flight/hotel cards, passkey approval card, booking confirmation |
| `start.sh` | Loads Paybox token, boots server (+ optional tunnel) |
...
- **Staging**: point at staging with `PAYBOX_MCP_URL=https://staging.api.paybox.sh/mcp PAYBOX_ISSUER=https://staging.api.paybox.sh` (re-run `paybox-auth.mjs`).
